...
<input type="text" name="c_name" value="<?php echo isset($arr->c_name)?$arr->c_name:""; ?>" required class="form-control">
					...
					<div class="form-group">
						...
						<select name="fk_category_product_id">
							<?php 
							 ...
							 ?>
								<option <?php if (isset($arr->fk_category_product_id)&&$arr->fk_category_product_id==$rows->pk_category_product_id) { ?> selected
								<?php } ?>
								value="<?php echo $rows->pk_category_product_id; ?>" > 
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-md-10">
	<input type="checkbox" <?php if (isset($arr->c_hotproduct) && $arr->c_hotproduct== 1) { ?>checked
	<?php } ?> name="c_hotproduct"> Tin nổi bật
							</div>
						</div>
					</div>
					<!-- end form group -->
